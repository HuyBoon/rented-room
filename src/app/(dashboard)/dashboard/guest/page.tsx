...
export default function GuestPage() {
  ...
  const fetchKhachThue = async (forceRefresh = false) => {
    try {
      ...
      if (selectedTrangThai && selectedTrangThai !== 'all') params.append('status', selectedTrangThai);
      
      const response = await fetch(`/api/tenants?${params.toString()}&limit=100`);
  ...
  const filteredKhachThue = khachThueList.filter(khachThue =>
    khachThue.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    khachThue.phoneNumber.includes(searchTerm) ||
    khachThue.idCardNumber.includes(searchTerm) ||
    khachThue.hometown.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-base md:text-2xl font-bold text-blue-600">
                {khachThueList.filter(k => k.status === 'renting').length}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-base md:text-2xl font-bold text-gray-600">
                {khachThueList.filter(k => k.status === 'movedOut').length}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-base md:text-2xl font-bold text-orange-600">
                {khachThueList.filter(k => k.status === 'idle').length}
              </p>
            </div>
        ...
        <div className="space-y-2 mb-4">
          ...
          <Select value={selectedTrangThai} onValueChange={setSelectedTrangThai}>
            ...
            <SelectContent>
              ...
              <SelectItem value="renting" className="text-sm">Đang thuê</SelectItem>
              <SelectItem value="movedOut" className="text-sm">Đã trả phòng</SelectItem>
              <SelectItem value="idle" className="text-sm">Chưa thuê</SelectItem>
            </SelectContent>
          </Select>
        </div>
            ...
            <Card key={khachThue._id} className="p-4">
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-medium text-gray-900">{khachThue.fullName}</h3>
                    <p className="text-sm text-gray-500 capitalize">{khachThue.gender}</p>
                  </div>
                  <div className="flex gap-2">
                    {(() => {
                      switch (khachThue.status) {
                        case 'renting':
                        ...
                        default:
                          return <Badge variant="outline" className="text-xs">{khachThue.status}</Badge>;
                      }
                    })()}
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{khachThue.phoneNumber}</span>
                  </div>
                  {khachThue.email && (
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <span className="font-mono">{khachThue.idCardNumber}</span>
                  </div>
                </div>
                ...
                <div className="space-y-1 text-xs text-gray-500">
                  <div className="flex items-center gap-2">
                    ...
                    <span>Ngày sinh: {new Date(khachThue.dateOfBirth).toLocaleDateString('vi-VN')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <MapPin className="h-3 w-3" />
                    <span className="truncate">{khachThue.hometown}</span>
                  </div>
                  {khachThue.occupation && (
                    <div className="flex items-center gap-2">
                      <Users className="h-3 w-3" />
                      <span>{khachThue.occupation}</span>
                    </div>
                  )}
                </div>

                {/* Room info if available */}
                {khachThue.currentContract?.roomId && (
                  <div className="border-t pt-2">
                    <div className="flex items-center gap-2 text-sm">
                      <Users className="h-3 w-3 text-green-600" />
                      <span className="font-medium">Phòng: {khachThue.currentContract.roomId.roomCode}</span>
                    </div>
                    {khachThue.currentContract.roomId.buildingId && (
                      <div className="flex items-center gap-2 text-xs text-gray-500 ml-5">
                        <span>{(khachThue.currentContract.roomId.buildingId as any).name}</span>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex justify-between items-center pt-2 border-t">
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        const publicUrl = `${window.location.origin}/tenants/login`;
...
function GuestForm({ 
  ...
  const [formData, setFormData] = useState({
    fullName: khachThue?.fullName || '',
    phoneNumber: khachThue?.phoneNumber || '',
    email: khachThue?.email || '',
    idCardNumber: khachThue?.idCardNumber || '',
    dateOfBirth: khachThue?.dateOfBirth ? new Date(khachThue.dateOfBirth).toISOString().split('T')[0] : '',
    gender: khachThue?.gender || 'male',
    hometown: khachThue?.hometown || '',
    idCardImages: {
      front: khachThue?.idCardImages?.front || '',
      back: khachThue?.idCardImages?.back || '',
    },
    occupation: khachThue?.occupation || '',
    password: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = khachThue ? `/api/tenants/${khachThue._id}` : '/api/tenants';
      const method = khachThue ? 'PUT' : 'POST';

      // Chỉ gửi password khi nó được nhập
      ...
      <Tabs defaultValue="thong-tin" className="w-full">
        ...
        <TabsContent value="thong-tin" className="space-y-4 md:space-y-6 mt-4 md:mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            <div className="space-y-2">
              <Label htmlFor="fullName" className="text-xs md:text-sm">Họ tên</Label>
              <Input
                id="fullName"
            ...
            <div className="space-y-2">
              <Label htmlFor="phoneNumber" className="text-xs md:text-sm">Số điện thoại</Label>
              <Input
                id="phoneNumber"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="idCardNumber" className="text-xs md:text-sm">CCCD</Label>
              <Input
                id="idCardNumber"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            <div className="space-y-2">
              <Label htmlFor="dateOfBirth" className="text-xs md:text-sm">Ngày sinh</Label>
              <Input
                id="dateOfBirth"
                type="date"
            ...
            <div className="space-y-2">
              <Label htmlFor="gender" className="text-xs md:text-sm">Giới tính</Label>
              <Select value={formData.gender} onValueChange={(value) => setFormData(prev => ({ ...prev, gender: value as 'male' | 'female' | 'other' }))}>
                <SelectTrigger className="text-sm">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="male" className="text-sm">Nam</SelectItem>
                  <SelectItem value="female" className="text-sm">Nữ</SelectItem>
                  <SelectItem value="other" className="text-sm">Khác</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="hometown" className="text-xs md:text-sm">Quê quán</Label>
            <Input
              id="hometown"
          ...
          <div className="space-y-2">
            <Label htmlFor="occupation" className="text-xs md:text-sm">Nghề nghiệp</Label>
            <Input
              id="occupation"
          ...
          <div className="space-y-2">
            <Label htmlFor="password" className="text-xs md:text-sm">Mật khẩu đăng nhập</Label>
            <Input
              id="password"
              type="password"
              value={formData.password}
              onChange={(e) => setFormData(prev => ({ ...prev, password: e.target.value }))}
              placeholder={khachThue && khachThue.password ? "Để trống nếu không muốn thay đổi" : "Nhập mật khẩu (tối thiểu 6 ký tự)"}
              className="text-sm"
            />
            <p className="text-[10px] md:text-xs text-muted-foreground">
              {khachThue && khachThue.password 
        ...
        <TabsContent value="anh-cccd" className="space-y-4 md:space-y-6 mt-4 md:mt-6">
          <CCCDUpload
            ...
            onImagesChange={(idCardImages: { front: string; back: string }) => setFormData(prev => ({ ...prev, idCardImages }))}
            className="w-full"
          />
        </TabsContent>
      </Tabs>
