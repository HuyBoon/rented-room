...
const getPhongName = (phongId: string | Phong) => {
  ...
  if (typeof phongId === 'object' && phongId.roomCode) {
    return phongId.roomCode;
  }
  return 'N/A';
};
...
const getKhachThueName = (khachThueId: string | KhachThue) => {
  ...
  if (typeof khachThueId === 'object' && khachThueId.fullName) {
    return khachThueId.fullName;
  }
  return 'N/A';
};
...
export default function PublicInvoicePage() {
  ...
  const fetchInvoiceData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/invoices-public/${hoaDonId}`);
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      tempElement.innerHTML = `
        ...
        ">
          ...
          <div style="text-align: center; margin-bottom: 30px;">
            ...
            <p style="font-size: 18px; margin: 10px 0 0 0; color: #333;">${hoaDon.invoiceCode}</p>
          </div>
          ...
          <div style="display: flex; gap: 30px; margin-bottom: 30px;">
            <div style="flex: 1;">
              ...
              <p style="margin: 5px 0; font-size: 14px;"><strong>Phòng:</strong> ${getPhongName(hoaDon.roomId)}</p>
              <p style="margin: 5px 0; font-size: 14px;"><strong>Khách thuê:</strong> ${getKhachThueName(hoaDon.tenantId)}</p>
            ...
            <div style="flex: 1;">
              ...
              <p style="margin: 5px 0; font-size: 14px;"><strong>Tháng/Năm:</strong> ${hoaDon.month}/${hoaDon.year}</p>
              <p style="margin: 5px 0; font-size: 14px;"><strong>Hạn thanh toán:</strong> ${new Date(hoaDon.dueDate).toLocaleDateString('vi-VN')}</p>
              <div style="margin: 10px 0;">
                ...
                ">
                  ${hoaDon.status === 'paid' ? 'Đã thanh toán' : 
                    hoaDon.status === 'partiallyPaid' ? 'Thanh toán một phần' : 
                    hoaDon.status === 'overdue' ? 'Quá hạn' : 'Chưa thanh toán'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div style="display: flex; gap: 20px;">
              <div style="flex: 1;">
                ...
                <div style="font-size: 13px; line-height: 1.6;">
                  <div style="display: flex; justify-content: space-between;"><span>Chỉ số ban đầu:</span><span>${hoaDon.electricityStart || 0} kWh</span></div>
                  <div style="display: flex; justify-content: space-between;"><span>Chỉ số cuối kỳ:</span><span>${hoaDon.electricityEnd || 0} kWh</span></div>
                  <div style="display: flex; justify-content: space-between; font-weight: bold;"><span>Số điện sử dụng:</span><span>${hoaDon.electricityUsage || 0} kWh</span></div>
                </div>
              </div>
              <div style="flex: 1;">
                <h4 style="font-size: 14px; font-weight: bold; margin-bottom: 10px; color: #555;">Nước</h4>
                <div style="font-size: 13px; line-height: 1.6;">
                  <div style="display: flex; justify-content: space-between;"><span>Chỉ số ban đầu:</span><span>${hoaDon.waterStart || 0} m³</span></div>
                  <div style="display: flex; justify-content: space-between;"><span>Chỉ số cuối kỳ:</span><span>${hoaDon.waterEnd || 0} m³</span></div>
                  <div style="display: flex; justify-content: space-between; font-weight: bold;"><span>Số nước sử dụng:</span><span>${hoaDon.waterUsage || 0} m³</span></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style="margin-bottom: 30px;">
            ...
            <div style="font-size: 14px; line-height: 1.8;">
              <div style="display: flex; justify-content: space-between;"><span>Tiền phòng</span><span>${formatCurrency(hoaDon.roomAmount)}</span></div>
              <div style="display: flex; justify-content: space-between;"><span>Tiền điện (${hoaDon.electricityUsage} kWh)</span><span>${formatCurrency(hoaDon.electricityAmount)}</span></div>
              <div style="display: flex; justify-content: space-between;"><span>Tiền nước (${hoaDon.waterUsage} m³)</span><span>${formatCurrency(hoaDon.waterAmount)}</span></div>
              ${hoaDon.serviceFees.map(phi => `
          ...
          <div style="border-top: 1px solid #000; padding-top: 20px;">
            <div style="display: flex; justify-content: space-between; font-size: 18px; font-weight: bold; margin-bottom: 10px;">
              <span>Tổng tiền:</span>
              <span>${formatCurrency(hoaDon.totalAmount)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 14px; margin-bottom: 5px;">
              <span>Đã thanh toán:</span>
              <span style="color: #10b981;">${formatCurrency(hoaDon.paidAmount)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 14px;">
              <span>Còn lại:</span>
              <span style="color: #10b981; font-weight: bold;">
                ${formatCurrency(hoaDon.remainingAmount)}
              </span>
            </div>
          </div>
        </div>
      ...
      // Tải xuống PDF
      pdf.save(`hoa-don-${hoaDon.invoiceCode}.pdf`);
        ...
        <div className="mb-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600">Mã hóa đơn: {hoaDon.invoiceCode}</p>
            </div>
            ...
            <Card>
              <CardHeader>
                <div className="text-center border-b pb-4">
                  ...
                  <p className="text-lg text-gray-600">{hoaDon.invoiceCode}</p>
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p><strong>Phòng:</strong> {getPhongName(hoaDon.roomId)}</p>
                    <p><strong>Khách thuê:</strong> {getKhachThueName(hoaDon.tenantId)}</p>
                    <p><strong>Hợp đồng:</strong> N/A</p>
                  </div>
                  <div>
                    <h3 className="font-semibold mb-2">Thông tin thanh toán</h3>
                    <p><strong>Tháng/Năm:</strong> {hoaDon.month}/{hoaDon.year}</p>
                    <p><strong>Hạn thanh toán:</strong> {new Date(hoaDon.dueDate).toLocaleDateString('vi-VN')}</p>
                    <p><strong>Trạng thái:</strong> {getStatusBadge(hoaDon.status)}</p>
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span>{hoaDon.electricityStart || 0} kWh</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Chỉ số cuối kỳ:</span>
                          <span>{hoaDon.electricityEnd || 0} kWh</span>
                        </div>
                        <div className="flex justify-between font-medium">
                          <span>Số điện sử dụng:</span>
                          <span>{hoaDon.electricityUsage || 0} kWh</span>
                        </div>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          <span>Chỉ số ban đầu:</span>
                          <span>{hoaDon.waterStart || 0} m³</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Chỉ số cuối kỳ:</span>
                          <span>{hoaDon.waterEnd || 0} m³</span>
                        </div>
                        <div className="flex justify-between font-medium">
                          <span>Số nước sử dụng:</span>
                          <span>{hoaDon.waterUsage || 0} m³</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span>{formatCurrency(hoaDon.roomAmount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Tiền điện ({hoaDon.electricityUsage} kWh)</span>
                      <span>{formatCurrency(hoaDon.electricityAmount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Tiền nước ({hoaDon.waterUsage} m³)</span>
                      <span>{formatCurrency(hoaDon.waterAmount)}</span>
                    </div>
                    {hoaDon.serviceFees.map((phi, index) => (
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="flex justify-between">
                    <span>Đã thanh toán:</span>
                    <span className="text-green-600">{formatCurrency(hoaDon.paidAmount)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Còn lại:</span>
                    <span className={hoaDon.remainingAmount > 0 ? 'text-red-600 font-semibold' : 'text-green-600'}>
                      {formatCurrency(hoaDon.remainingAmount)}
                    </span>
                  </div>
                </div>
                ...
                {hoaDon.notes && (
                  <div>
                    ...
                    <p className="text-gray-600">{hoaDon.notes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-center">
                      <img
                        src={`https://img.vietqr.io/image/970422-7320012003-compact2.png?amount=${hoaDon.remainingAmount}&addInfo=Thanh%20toan%20${hoaDon.invoiceCode}&accountName=CHU%20TRO`}
                        alt="VietQR Code"
                      ...
                      <div key={thanhToan._id} className="border rounded-lg p-3">
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex items-center gap-2">
                            {getMethodBadge(thanhToan.method)}
                          </div>
                          <span className="font-semibold text-green-600">
                            {formatCurrency(thanhToan.amount)}
                          </span>
                        </div>
                        <div className="text-sm text-gray-600 space-y-1">
                          ...
                          {thanhToan.transferInfo && (
                            <div>
                              <div>{thanhToan.transferInfo.bank}</div>
                              <div className="text-xs text-gray-500">
                                {thanhToan.transferInfo.transactionId}
                              </div>
                            </div>
                          )}
                          {thanhToan.notes && (
