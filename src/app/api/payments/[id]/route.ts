...
export async function PUT(
  ...
  try {
    ...
    const thanhToanHienTai = await Payment.findById(id);
    if (!thanhToanHienTai) {
      return NextResponse.json(
    ...
    const hoaDon = await Invoice.findById(hoaDonId);
    if (!hoaDon) {
      return NextResponse.json(
    ...
    const hoaDonCu = await Invoice.findById(thanhToanHienTai.hoaDon);
    if (hoaDonCu) {
...
export async function DELETE(
  ...
  try {
    ...
    // Tìm thanh toán
    const thanhToan = await Payment.findById(id);
    if (!thanhToan) {
      return NextResponse.json(
        { message: 'Thanh toán không tồn tại' },
        { status: 404 }
    ...
    const hoaDon = await Invoice.findById(thanhToan.hoaDon);
    if (hoaDon) {
